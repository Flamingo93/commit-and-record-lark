...
2. 创建 14 个字段（repository, commit_message, session_model, commit_hash, branch, author, author_email, commit_time, lines_added, lines_deleted, files_changed, session_cost, session_input_tokens, session_output_tokens）
3. 为当前用户授予管理员权限
...
session_cost 通过解析 Claude Code 的 session transcript (JSONL) 或 Codex session 文件，按模型分别计算后汇总。Claude 定价如下（单位：美元/百万 token）：
...
在 Codex 环境下，脚本会记录 `session_model`，并根据内置的 OpenAI GPT/Codex 价格表估算费用。每次记录只计算自上次记录以来新增的 token 消耗（增量计算），通过 `.last-offset` 文件跟踪偏移量和累计状态。
